// ============================================================
// DECO PLANNER - B├ťHLMANN ZHL-16C ALGORITHM DATA
// ============================================================

/**
 * B├╝hlmann ZHL-16C Compartment Data for Nitrogen (N2)
 * Based on: "Tauchmedizin" by Albert A. B├╝hlmann
 * 
...
/**
 * B├╝hlmann ZHL-16C Compartment Data for Helium (He)
 ...
 * P_amb = P_atm + (depth ├Ś water_pressure_gradient)
 */
 ...
 * P_insp = (P_amb - P_water_vapor) ├Ś FiGas
 * 
 * Simplification: P_water_vapor Ôëł 0 (ignored for deco calculations)
 * P_insp = P_amb ├Ś FiGas
 */
 ...
 * 
 * P_tissue(t) = P_insp + R ├Ś (t - 1/k) - [P_insp - P_initial - R/k] ├Ś e^(-k├Śt)
 * 
 * Where:
 ...
 * P_tissue(t) = P_initial + (P_insp - P_initial) ├Ś (1 - e^(-k├Śt))
 */
 ...
 * M-value_GF = M-value_0 + GF ├Ś (M-value - M-value_0)
 * 
...
function getCeilingDepth(tissuePressure, compartmentIndex, gf, useHelium = false) {
    ...
    // M = P_amb/b + a
    // M_GF = P_amb + GF * (M - P_amb)
    // P_tissue = P_amb + GF * (P_amb/b + a - P_amb)
    // P_tissue = P_amb * (1 - GF + GF/b) + GF * a
    // P_tissue - GF * a = P_amb * (1 - GF + GF/b)
    // P_amb = (P_tissue - GF * a) / (1 - GF + GF/b)
    //
    // Multiply by b/b to simplify:
    // P_amb = (P_tissue - GF * a) * b / (b * (1 - GF) + GF)

    const gfFraction = gf / 100;
    const pAmb = ((tissuePressure - a * gfFraction) * b) / (gfFraction + b * (1 - gfFraction));
